...
#define BLE_UUID_DEVINFO_CHARACTERISTIC_COMPUTERS		0x1606
...
void register_devinfo_callbacks(devinfo_callbacks_t *callbacks)
{
    ...
    if(m_callbacks->on_computers_update == NULL)
    {
        m_callbacks->on_computers_update = (callback_t)devinfo_callback_is_null;
    }
...
static void on_write(ble_devinfo_t * p_devinfo, ble_evt_t * p_ble_evt)
{
    ...
    else if (p_evt_write->handle == p_devinfo->computers_handles.value_handle)
    {
        m_callbacks->on_computers_update(p_evt_write->data, p_evt_write->len);
    }
...
static uint32_t computers_characteristic_add(ble_devinfo_t * p_devinfo, const ble_devinfo_init_t * p_devinfo_init)
{
	/**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_t    attr_char_value;
    ...
    ble_uuid.type = p_devinfo->uuid_type;
    ble_uuid.uuid = BLE_UUID_DEVINFO_CHARACTERISTIC_COMPUTERS;
...
uint32_t ble_devinfo_init(ble_devinfo_t * p_devinfo, const ble_devinfo_init_t * p_devinfo_init)
{
	...
	err_code = computers_characteristic_add(p_devinfo, p_devinfo_init);
	if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
